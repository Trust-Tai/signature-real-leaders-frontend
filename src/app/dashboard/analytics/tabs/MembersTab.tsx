...
const MembersTab = () => {
  ...
  const transformFollowerData = useCallback((apiFollower: {
    ...
    return {
      ...
      age,
      followedAt,
      location: apiFollower.company_name || 'Unknown',
      joined: followedAt,
    };
  }, []);
  ...
  if (error && !followersData.length) {
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <p className="text-red-600">{error}</p>
            <button onClick={() => fetchFollowers()} className="text-red-400 hover:text-red-600 text-sm underline">
              Try Again
            </button>
          </div>
        </div>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <h2 className="text-xl font-bold text-[#101117]">Members</h2>

      <StatsCards stats={statsCards} columns={4} />

      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <button onClick={handleExportFollowers} disabled={exportLoading} className="bg-white text-[#CF3232] border border-[#CF3232] px-4 py-2 rounded-lg hover:bg-red-50 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
          <Download className="w-4 h-4" />
          <span>{exportLoading ? 'Exporting...' : 'Export Members'}</span>
        </button>
        <button onClick={() => { setFilterSearchTerm(searchTerm); setFilterOpen(true); }} className="bg-white text-gray-600 border border-gray-300 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {filtered.map((f) => (
            <div key={f.id} className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">Follower</span>
              </div>
        ...
        <div className="p-4 sm:p-6 border-t border-gray-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <p className="text-sm text-gray-600">Showing {filtered.length} of {totalFollowers} members</p>
            
            {totalPages > 1 && (
              <div className="flex items-center space-x-2">
                <button onClick={() => { if (isSearchMode) { searchFollowers(searchTerm); } else { fetchFollowers(currentPage - 1); } }} disabled={currentPage === 1 || loading || searchLoading} className="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                  Previous
                </button>
                <span className="text-sm text-gray-600">Page {currentPage} of {totalPages}</span>
                <button onClick={() => { if (isSearchMode) { searchFollowers(searchTerm); } else { fetchFollowers(currentPage + 1); } }} disabled={currentPage === totalPages || loading || searchLoading} className="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                  Next
                </button>
              </div>
            )}
      ...
      {filterOpen && (
        <div className="fixed inset-0 bg-opacity-[0.3] z-40 flex items-center justify-center p-4 transition-opacity duration-300" onClick={() => setFilterOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto relative z-50 transform transition-all duration-300 scale-100" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-[#101117]">Filter Members</h2>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <input type="text" placeholder="Search by name, email, company, or occupation..." value={filterSearchTerm} onChange={(e) => setFilterSearchTerm(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CF3232] focus:border-transparent font-outfit" style={{ color: '#949494' }} />
              </div>
            </div>
            <div className="p-6 border-t border-gray-200 flex gap-3">
              <button onClick={() => { setFilterSearchTerm(''); setSearchTerm(''); setIsSearchMode(false); fetchFollowers(); setFilterOpen(false); }} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                Reset
              </button>
              <button onClick={() => { setSearchTerm(filterSearchTerm); if (filterSearchTerm.trim()) { searchFollowers(filterSearchTerm); } else { setIsSearchMode(false); fetchFollowers(); } setFilterOpen(false); }} disabled={searchLoading} className="flex-1 px-4 py-2 bg-[#CF3232] text-white rounded-lg hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
